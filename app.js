...
async function main() {
  ...
  var fichero = await leerFichero('./input/f_libraries_of_the_world.txt')
  getMetaData(fichero)
  ...
  let lineasSalida = []
  lineasSalida.push(String(librerias))

  // Buscamos la primera libreria
  let libreriasMenorMayor = arrayLibrerias
  libreriasMenorMayor.sort((a, b) => {
    return a.cantidadDias - b.cantidadDias
  })

  for (let libreria of libreriasMenorMayor) {
    let fline = ''
    fline += libreria.id + ' '
    fline += libreria.cantidadLibros
    lineasSalida.push(fline)

    let sline = ''
    for (let libro of libreria.libros) {
      sline += libro.id + ' '
    }
    lineasSalida.push(sline)
  }

  for (let line of lineasSalida) {
    console.log(line)
  }

  //escribirSalida(lineasSalida, 'ou')
}
